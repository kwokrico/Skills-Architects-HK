---
name: hk-unauthorised-building-works
description: Activate for unauthorised building works (UBW) in Hong Kong, including identification and classification of non-compliant works, BD removal and repair orders (s.24/s.25), enforcement actions, rectification and regularization pathways, and implications for sale, occupation permit, and consent applications.
disable-model-invocation: true
---

# HK Unauthorised Building Works (HK-UBW)
Supports assessment and handling of UBW matters in Hong Kong practice, from initial identification through enforcement response, compliance strategy, and transaction-stage implications.

---

For **UBW identification and remediation**, use `hk-unauthorised-building-works`. For other topics, see the routing table below.

## When to Use This Skill

UBW, BD s.24/s.25 orders, regularization, sale/OP implications.

| Question type | Use this skill | Use instead |
|---------------|----------------|-------------|
| UBW identification and remediation | `hk-unauthorised-building-works` | `—` |
| MWCS validation scheme items | `—` | `hk-minor-works` |
| OP with UBW risk | `—` | `hk-op-submission-strategy` |

## Halt criteria

- Do not guarantee regularization outcome.
## 1. Scope and Practical Definition
In this context, UBW generally means building works carried out without required approval, consent, exemption, or in deviation from approved plans under the Buildings Ordinance framework.

Core risk areas:
- Structural safety and loading changes.
- Means of escape and fire safety impairment.
- Drainage and waterproofing defects.
- Encroachment into common parts or government land.
...
## 3. BD Orders and Statutory Response (s.24 / s.25)
When BD issues enforcement directions, treat deadlines and scope strictly.

- **s.24 context:** removal/correction orders for unauthorized works.
- **s.25 context:** repair-related orders for dangerous or defective conditions.
...
3. **Minor Works route** where applicable and legally valid for the specific scope.
...
UBW may materially affect conveyancing, lending, and regulatory approvals.
...
- **OP/completion stage:** unresolved non-compliance can delay OP-related progression and acceptance.
- **Consent applications/new works:** existing UBW can complicate fresh consent strategy and sequence.
...
*Reference anchors for live projects: Buildings Ordinance Cap. 123 and current BD enforcement guidance, including applicable order language and latest procedural circulars.*
