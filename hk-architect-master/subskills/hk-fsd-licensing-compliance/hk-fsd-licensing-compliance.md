---
name: hk-fsd-licensing-compliance
description: Activate for Fire Services handover and certification in Hong Kong projects, including FSI/314 and FSI/501 workflows, FSD inspections, hydrant/hose reel testing, and Fire Certificate (FS 172) readiness.
disable-model-invocation: true
---

# HK FSD Licensing & Compliance
Execution guide for closing out fire services installations and obtaining authority acceptance at handover stage in Hong Kong.

---

For **FSD handover and Fire Certificate**, use `hk-fsd-licensing-compliance`. For other topics, see the routing table below.

## When to Use This Skill

FSI/314, FSI/501, FSD inspection, hydrant/hose reel tests, Fire Certificate FS 172.

| Question type | Use this skill | Use instead |
|---------------|----------------|-------------|
| FSD handover and Fire Certificate | `hk-fsd-licensing-compliance` | `—` |
| Building fire strategy design | `—` | `hk-fire-life-safety` |
| MEP design coordination | `—` | `hk-building-services` |
## 1. Scope and Outcome
- This sub-skill focuses on the practical route from installed fire systems to authority sign-off.
- It centers on:
  - Form `FSI/314` and form `FSI/501` certifications.
  - Fire hydrant/hose reel testing and associated records.
  - Fire Services Department (FSD) inspection readiness and follow-up.
  - Fire Certificate (`FS 172`) application package quality.
...
3. Prepare certification package (`FSI/314`, `FSI/501`, test logs, as-built drawings).
4. Submit for FSD inspection booking.
5. Conduct inspection and close comments/punch items immediately.
6. Re-submit rectification evidence where required.
7. Finalize Fire Certificate (`FS 172`) documentation and issuance process.

---

## 3. Forms and Documents (Core)
### `FSI/314`
- Use for installation and compliance declaration under the relevant project scope.
- Ensure item descriptions exactly match installed system descriptions and approved plans.
- Keep signer details, registration references, and dates consistent across all forms.

### `FSI/501`
- Use for completion/certification stage submission tied to final acceptance workflow.
- Ensure all referenced test sheets and annexes are attached and clearly indexed.
- Any discrepancy between `FSI/501` and test records is a high-probability inspection issue.

### `FS 172` (Fire Certificate)
- Build the submission pack only after all inspection comments are closed out.
- Include latest approved/as-built records and complete testing evidence.
- Keep document naming and revision control clean to avoid administrative rejection.

---

## 4. Inspection Readiness Checklist (FSD)
...
- Incomplete annexes to `FSI/314`/`FSI/501`.
...
- Completed `FSI/314` and `FSI/501` (latest signed versions).
- Final fire services as-built drawings.
...
- Final `FS 172` submission bundle.
...
5. `FS 172` readiness verdict with blockers clearly stated.

---

*Use this guidance with project-specific approved plans, contract requirements, and current FSD procedural expectations.*
