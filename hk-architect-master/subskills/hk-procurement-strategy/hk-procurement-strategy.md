...
- Do not recommend a procurement route without confirming client capacity, programme, and design maturity (see §2).
- Typhoon EOT entitlements are route-specific — cite the typhoon reference; do not generalize across contract forms.
...
## 8A. Construction sequencing — procurement swimlane

The diagram **lane 9** (需求提出 → 材料進場) runs **parallel** to superstructure from tender award — not after structure completes. Long-lead items (curtain wall, lifts, switchgear) must sit on the master programme critical path. Route choice (D&B, MC, traditional) changes how lane 9 is administered but not the need for early sample and shop-drawing approval. Load `hk-construction-programme` for the full 10-lane map.
